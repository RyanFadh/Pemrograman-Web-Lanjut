...
<nav class="navbar navbar-expand-lg navbar-dark" ; style="background-color: red;">
    ...
    <form action="<?= base_url('Arsenal/editPemain') . '/' .  $pemain['id_pemain']?>" method="POST" enctype="multipart/form-data" style="width: 20cm;">
        <div class="form-group">
            <label class="badge badge-light text-wrap" style="font-size: 15px; font-weight: bold; color: #8B0000;">ID</label>
            <input name="id_pemain" class="form-control" type="text" placeholder="Masukkan ID" value="<?= $pemain['id_pemain']?>">
        </div>
            <div class="form-group">
            <label class="badge badge-light text-wrap" style="font-size: 15px; font-weight: bold; color: #8B0000;">Nama</label>
            <input name="nama" class="form-control" type="text" placeholder="Masukkan Nama" value="<?= $pemain['nama']?>">
        </div>
        <div class="form-group">
            <label class="badge badge-light text-wrap" style="font-size: 15px; font-weight: bold; color: #8B0000;">Posisi</label>
            <input name="posisi" class="form-control" type="text" placeholder="Masukkan Posisi" value="<?= $pemain['posisi']?>">
        </div>
        <label class="badge badge-light text-wrap" style="font-size: 15px; font-weight: bold; color: #8B0000;">Negara</label>
        <div class="input-group mb-3">
        <select class="custom-select" name="id_negara" value="<?= $pemain['id_negara']?>">
            <option selected>Silahkan Pilih</option>
            <?php
                foreach ($negara as $row) {
            ?>
        <option value="<?= $row['id_negara'] ?>" <?= ($pemain['id_negara'] == $row['id_negara']) ? 'selected' : ''; ?>><?=$row['nama_negara']?></option>
            <?php } ?>
        </select>
        </div>
